...
def dataOutGlob():
    for file in g.glob('*MISSOULA_URBANS.csv'): # This was run on Missoula Montana data
...
def headerBody():
    ...
    try:
        ...
        try:
            while True:
                yield str(dist[b][0:])[1:-1].replace("'","") # strips away characters
                b = b + 1
                continue
        except IndexError: sys.exit(0)

def fileWriter(): # outputs data into csv file
    # column headers
    columns=('dStart','dEnd','CORRIDOR_RB','ROAD_PATHWAY','VAN_NO',\
             'BEG_GN','END_GN','CORRIDOR','SECFILE',\
             'COUNTY_NAME','DISTRICT','ROAD_VAN','FROM_DESCR','TO_DESCR',\
             'FRFPOST','TRFPOST','BEGIN_MI','END_MI','DIR','LENGTH','LANE',\
             'ROADBED','START_LONG','START_LAT','END_LONG','END_LAT','PAVETYPE')
    
    outfile = csv.writer(open('UrbansSorted.csv','wb'))
