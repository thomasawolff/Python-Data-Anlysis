...
def headerBody():
    ...
    for n in range(1,len(data1)):
        ...
        try:
            xSd = sin(yDistWGS/2)**2 + cos(float(data1[n][20]))*cos(float(data1[n][22]))*sin((xDistWGS/2))**2
            ySd = 2*atan2(sqrt(xSd),sqrt(1 - xSd))
            dDist = 3958.756 * ySd # distance in miles between start and end nodes
        
            if diffLatS > 0.0 and diffLongS > 0.0 and data1[4] != '1E+12' and data1[3] != '1E+12':
    
                xS = sin(diffLatS/2)**2 + cos(float(data1[m][20]))*cos(float(data1[n][20]))*sin(diffLongS/2)**2
                yS = 2*atan2(sqrt(xS),sqrt(1 - xS))
                dStart = 3958.756 * yS # distance in miles between starting nodes of different rows
                
                xE = sin(diffLatE/2)**2 + cos(float(data1[m][22]))*cos(float(data1[n][22]))*sin(diffLongE/2)**2
...
##                yield round((math.sqrt(xDiffTis+yDiffTis)*3.280)/5280.0,4),round(dDist,4),\
##                      data1[n][0],data1[n][9],data1[n][10],data1[n][11],\
##                      round(float(data1[n][14]),4),round(float(data1[n][15]),4)
                
                yield dStart,dEnd,dDist,data1[n][0],data1[n][1],int(data1[n][2]),\
        ...
        for d in range(0,len(dist)):
            if (dist[w][6] != '1E+12' and dist[d][5] != '1E+12' and\
               ...
               (int(dist[w][6]) == int(dist[d][5])\
                ...
                dist[w][2] != dist[d][2] and dist[w][18] == 'I':
                pass
##                yield dist[w][2],float(dist[w][16]),float(dist[w][17]),int(dist[w][5]),int(dist[w][6]),\
##                      dist[d][2],dist[d][11],int(dist[d][5]),int(dist[d][6])
##                        int(dist[w][5]),int(dist[w][6]),dist[w][11],\
##                        dist[w][12],dist[w][13],dist[w][14],dist[w][15]
            else:
                pass
